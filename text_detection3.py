import cv2
import numpy as np
import pytesseract
pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"

img = cv2.imread("board2.jpg")
#gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
#blur = cv2.GaussianBlur(gray, (3,3), 0)
#thresh = cv2.threshold(blur, 120, 255, cv2.THRESH_BINARY_INV)[1]
#gimp hsv (29,27.4, 85.9) -> OpenCV hsv (14.5, 70, 217) 

hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
thresh = cv2.inRange(hsv, (14.5-5, 70-30, 217-10) , (14.5+5, 70+30, 217+30))
#on trouve le beige dans l'image

# items = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
# contours = items[0] if len(items) == 2 else items[1]

# print(len(contours))
# img_contour = img.copy()
# for i in range(len(contours)):
#     area = cv2.contourArea(contours[i])
#     if 30 < area < 10000:
#         cv2.drawContours(img_contour, contours, i, (0, 0, 255), 2)

# detected = ""
# for c in contours:
#     x, y, w, h = cv2.boundingRect(c)
#     ratio = h/w
#     area = cv2.contourArea(c)
#     base = np.ones(thresh.shape, dtype=np.uint8)
#     if ratio > 0.7 and 30 < area < 10000:
#         base[y:y+h, x:x+w] = thresh[y:y+h, x:x+w]
#         segment = cv2.bitwise_not(base)

#         custom_config = r'-l eng --psm 10 -c tessedit_char_whitelist="ABCDEFGHIJKLMNOPQRSTUVWXYZ" '
#         c = pytesseract.image_to_string(segment, config=custom_config)
#         print(c)
#         detected = detected + c

# print("detected: " + detected)

hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
thresh = cv2.inRange(hsv, (14.5-5, 70-30, 217-10) , (14.5+5, 70+30, 217+30))
print(np.count_nonzero(thresh))
cv2.imshow("img_contour", thresh)

cv2.waitKey(0)
cv2.destroyAllWindows()